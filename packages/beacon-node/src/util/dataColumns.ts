...
import {ColumnIndex, CustodyIndex, deneb, fulu, SignedBeaconBlockHeader} from "@lodestar/types";
...
export function computeCustodyConfig(nodeId: NodeId, config: ChainForkConfig): CustodyConfig {
  ...
  // );
  const custodyGroupCount = Math.max(
  ...
  const sampledColumns = sampleGroups.flatMap(computeColumnsForCustodyGroup).sort((a, b) => a - b);
